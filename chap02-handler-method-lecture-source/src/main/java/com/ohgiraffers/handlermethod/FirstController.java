...
public class FirstController {
     ...
     *   => /first/regist 요청 들어오면 /first/regist 뷰를 응답한다.
     *  */
    @GetMapping("/regist")
    public void regist() {
    }
     ...
     *   WebRequest 는 HttpServletRequest 의 정보를 대부분 가지고 있는
     *   API 로 Servlet 에 종속적이지 않다.
    ...
    *   @ModelAttribute("모델에 담을 key 값") 이름을 정하지 않았다면
    *   -> 타입의 앞글자를 소문자로 한 네이밍 규칙을 따른다.
    *   생략이 가능하지만 명시적으로 작성하는 것이 좋음
    * */
    ...
    *  */
    ...
    @PostMapping("body")
    /* @RequestBody
    *   http 본문 자체 읽는 부분을 모델로 변환시켜 주는 어노테이션
